...
type Kube interface {
    ...
    GetSecret(ctx context.Context, string, namespace string) (*corev1.Secret, error)
...
// GetSecret fetches a specific secret within a specified namespace
func (k *KubeClient) GetSecret(ctx context.Context, secretName, namespace string) (*corev1.Secret, error) {
    out, err := k.runKubectlCmd([]string{"get", "secret", secretName, "-n", namespace, "-o", "json"})
    if err != nil {
        if strings.Contains(string(out), "not found") {
            return nil, nil
        }
...
func (k *KubeClient) GetApplication(ctx context.Context, name types.NamespacedName) (*wego.Application, error) {
    ...
    if err != nil {
        return nil, fmt.Errorf("could not run kubectl command: %w", err)
    }
...
func (k *KubeClient) GetApplications(ctx context.Context, ns string) ([]wego.Application, error) {
    ...
    if err := json.Unmarshal(output, &a); err != nil {
        return nil, fmt.Errorf("could not unmarshal applications json: %w", err)
    }
