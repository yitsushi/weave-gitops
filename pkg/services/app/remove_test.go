...
func updateAppFromParams() error {
	params, err := appSrv.(*AppSvc).updateParametersIfNecessary(context.Background(), localAddParams)
	if err != nil {
		return err
	}
...
var _ = Describe("Remove", func() {
	...
	It("Looks up config repo default branch", func() {
		...
		repoUrl, branch, err := appSrv.(*AppSvc).getConfigUrlAndBranch(context.Background(), app)
		Expect(err).ShouldNot(HaveOccurred())
