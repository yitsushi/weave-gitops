...
func (a *AppSvc) Add(params AddParams) error {
	...
	switch strings.ToUpper(app.Spec.ConfigURL) {
	case string(models.ConfigTypeNone):
		return a.addAppWithNoConfigRepo(app, params.DryRun, clusterName)
	case string(models.ConfigTypeUserRepo):
		return a.addAppWithConfigInAppRepo(ctx, app, params, clusterName)
	default:
		return a.addAppWithConfigInExternalRepo(ctx, app, params, clusterName)
	}
}
...
func (a *AppSvc) updateParametersIfNecessary(ctx context.Context, params AddParams) (AddParams, error) {
...
func (a *AppSvc) addAppWithConfigInAppRepo(ctx context.Context, app models.Application, params AddParams, clusterName string) error {
	manifests, err := a.Automation.GenerateManifests(context.Background(), app, clusterName)
	if err != nil {
		return fmt.Errorf("could not generate application GitOps Automation manifests: %w", err)
	}
	...
	if !params.DryRun {
		if !params.AutoMerge {
			if err := a.createPullRequestToRepo(ctx, app, app.AppRepoUrl, manifests); err != nil {
				return err
			}
		} else {
...
func (a *AppSvc) addAppWithConfigInExternalRepo(ctx context.Context, app models.Application, params AddParams, clusterName string) error {
	manifests, err := a.Automation.GenerateManifests(context.Background(), app, clusterName)
	if err != nil {
		return fmt.Errorf("could not generate application GitOps Automation manifests: %w", err)
	}
	...
	if !params.DryRun {
		if !params.AutoMerge {
			if err := a.createPullRequestToRepo(ctx, app, app.ConfigRepoUrl, manifests); err != nil {
				return err
			}
		} else {
			a.Logger.Actionf("Writing manifests to disk")
